...
show_menu() {
    while true; do
        ...
        echo ""
        echo -e "${GREEN}1)${NC} ${PACKAGE} Install All (Recommended) ${CYAN}→ Everything: updates, essentials, monitoring, security, database, GPS${NC}"
        echo -e "${GREEN}2)${NC} ${GEAR} System Configuration Only ${CYAN}→ Expand filesystem, enable SSH/VNC, set WiFi region${NC}"
        echo -e "${GREEN}3)${NC} ${MONITOR} Monitoring Stack (Grafana + InfluxDB) ${CYAN}→ Time-series database + visualization dashboard${NC}"
        echo -e "${GREEN}4)${NC} ${SECURITY} Network Security Tools ${CYAN}→ nmap, wireshark, aircrack-ng, bettercap, kismet${NC}"
        echo -e "${GREEN}5)${NC} ${DATABASE} Database (PostgreSQL) ${CYAN}→ PostgreSQL server with contrib modules${NC}"
        echo -e "${GREEN}6)${NC} ${NETWORK} GPS Tools ${CYAN}→ GPS daemon and tools for device connectivity${NC}"
        echo -e "${GREEN}7)${NC} ${TOOLS} Essential Packages Only ${CYAN}→ curl, wget, git, htop, nano, vim, tree, jq, dialog${NC}"
        echo -e "${GREEN}8)${NC} Custom Selection ${CYAN}→ Choose specific components from detailed menu${NC}"
        echo -e "${GREEN}9)${NC} Exit ${CYAN}→ Exit without making changes${NC}"
        echo ""
